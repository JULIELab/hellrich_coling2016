...
def main():
    ...
    if len(sys.argv) < 12:
        raise Exception("""Provide 5+ arguments:\n\t1,path to save models\n\t2,path to corpora
            ...
            \n\t10,alpha
            \n\t11+ files to train on (one model per file)""")
    ...
    for f in files:
        ...
        while epoch < epochs:
            ...
            model.save_word2vec_format(
                    fname + "_epoch_" + str(epoch), fvocab=fvocab + "_epoch_" + str(epoch), binary=True)
            model.save(fname+ "_resumable_epoch_" + str(epoch))
            logging.info("finished epoch %s with distance %s", epoch, dist)
        old_model = model
        model.save_word2vec_format(fname, fvocab=fvocab, binary=True)
        model.save(fname+ "_resumable")
